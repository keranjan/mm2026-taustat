...
async function loadResults() {
  try {
    const res = await api('staff_results?select=match_id,home_goals,away_goals');
    if (!res.ok) return;
    ...
    check: ({ preds }) => {
      return MATCHES.some(m => {
        ...
        const othersExact = Object.values(users).filter(udata => {
          const op = udata.staff_predictions?.[m.id];
...
async function stepResult(id, side, delta) {
  ...
  if (h === null && a === null) {
    await api(`staff_results?match_id=eq.${id}`, { method: 'DELETE' });
    delete results[id];
  } else if (h !== null && a !== null) {
    await api('staff_results', {
      method: 'POST',
