...
function drawBracketConnectors(treeId, svgId, idPrefix) {
  ...
  const getRight = el => {
    ...
    return { x: r.right - treeRect.left, y: r.top + r.height / 2 - treeRect.top };
  };
  const getLeft = el => {
    const r = el.getBoundingClientRect();
    return { x: r.left - treeRect.left, y: r.top + r.height / 2 - treeRect.top };
  };
  ...
  // Yhdistäjäfunktio: ryhmä lähtöelementtejä → yksi kohde
  function connectGroup(srcs, dst) {
    const d      = getLeft(dst);
    const points = srcs.map(getRight);
    const midX   = points[0].x + (d.x - points[0].x) * 0.5;
    const ys     = points.map(p => p.y);
    const minY   = Math.min(...ys);
    const maxY   = Math.max(...ys);
    // Vaakaviivat lähtöpisteistä midX:ään
    points.forEach(p => paths.push(`M ${p.x} ${p.y} H ${midX}`));
    // Pystyviiva midX:llä yhdistämään kaikki lähtöpisteet
    if (minY < maxY) paths.push(`M ${midX} ${minY} V ${maxY}`);
    // Vaakaviiva midX:ltä kohteen Y-tasolta kohteeseen
    paths.push(`M ${midX} ${d.y} H ${d.x}`);
  }

  // R16 pareittain → QF (r16_1+r16_2→qf1, r16_3+r16_4→qf2, r16_5+r16_6→qf3, r16_7+r16_8→qf4)
  connectGroup([r16[0], r16[1]], qf[0]);
  connectGroup([r16[2], r16[3]], qf[1]);
  connectGroup([r16[4], r16[5]], qf[2]);
  connectGroup([r16[6], r16[7]], qf[3]);

  // QF pareittain → Finalistit (qf1+qf2→f1, qf3+qf4→f2)
  connectGroup([qf[0], qf[1]], f[0]);
  connectGroup([qf[2], qf[3]], f[1]);
